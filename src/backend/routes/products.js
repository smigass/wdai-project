...
router.get('/:id', (req, res) => {
  db.get(`SELECT * FROM Products WHERE ProductID = ?;`, [req.params.id], (err, product) => {
    if (err) {
...
router.post('/', authenticateJWT, (req, res) => {
  ...
  db.run(
    `INSERT INTO Products (Name, Description, Price, Image, InStock, CategoryID) VALUES (?, ?, ?, ?, ?, ?);`,
...
router.get('/category/:categoryId', (req, res) => {
  ...
  db.all(`SELECT * FROM Products WHERE CategoryID = ?;`, [categoryId], (err, products) => {
    if (err) {
...
router.get('/bestsellers', (req, res) => {
  db.all(
    `SELECT * FROM Products ORDER BY OrdersCount DESC LIMIT 10;`,
    [],
    (err, products) => {
      if (err) {
